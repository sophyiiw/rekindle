...
def menu_pembeli(user_login):
    while True:
        ...
        if pilih == "1":
            ...
            print("\n--- KASIR REKINDLE ---")
            total_belanja = 0
            total_qty = 0 # Untuk hitung jumlah barang buat diskon
            # 1. Tampilkan Rincian
            ...
            print(f"Total Awal   : Rp {int(total_belanja)}")
            print(f"Total Barang : {total_qty} pcs")
            if persen_diskon > 0:
                print(f"DISKON {persen_diskon}%   : -Rp {int(potongan_harga)}")
            else:
                print("DISKON       : - (Beli min 3 dpt diskon!)")
            print(f"TOTAL BAYAR  : Rp {int(total_akhir)}")
            print("------------------------------")
            # 4. Konfirmasi Bayar
            if input("Bayar sekarang? (y/n): ") == "y":
                for item in keranjang:
                    ...
                    riwayat_transaksi.append({
                        ...
                        "barang": item['nama'],
                        "qty": item['qty'],
                        "total": item['harga'] * item['qty'], # Simpan harga asli di history (opsional)
                        "status": "Diproses"
                    })
                print(">> Pembayaran LUNAS! Terima kasih.")
                keranjang.clear()
            else: print(">> Pembayaran dibatalkan.")
            ...
            print("\n--- PUSAT BANTUAN ---")
            print("1. Tulis Laporan Baru")
            print("2. Cek Riwayat & Balasan Admin")
            sub_menu = input("Pilih: ")
            # TULIS BARU
            if sub_menu == "1":
                pesan_user = input("Tulis keluhan/pertanyaan Anda: ")
                inbox_laporan.append({
                    "pengirim": user_login,
                    "pesan": pesan_user,
                    "jawaban": "Belum dibalas" # Default jawaban
                })
                print(">> Laporan terkirim ke Admin.")
            # LIHAT BALASAN
            elif sub_menu == "2":
                print("\n--- RIWAYAT LAPORAN SAYA ---")
                ada_laporan = False
                # Cek semua pesan di database
                for chat in inbox_laporan:
                    # Filter: Cuma tampilkan pesan milik user yang lagi login
                    if chat['pengirim'] == user_login:
                        print(f"Q (Anda) : {chat['pesan']}")
                        print(f"A (Admin): {chat['jawaban']}")
                        print("-" * 30)
                        ada_laporan = True
                if not ada_laporan:
                    print(">> Belum ada riwayat laporan.")
